...
data = blueprinty.to_pandas()
customers = data[data["iscustomer"] == 1]["patents"]
non_customers = data[data["iscustomer"] == 0]["patents"]

# Calculate means
mean_cust = customers.mean()
mean_non_cust = non_customers.mean()
...
axes[0].axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Mean: {mean_cust:.2f}",
)
axes[0].set_title("Number of Patents for Customers")
...
axes[1].axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"Mean: {mean_non_cust:.2f}",
)
axes[1].set_title("Number of Patents for Non-Customers")
...
#| output: true
data = data
# regions by customer status
region_counts = data.groupby(["iscustomer", "region"]).size().unstack(fill_value=0)
print(region_counts)
#plot histograms of region by customer status
plt.figure(figsize=(10, 6))
sns.countplot(x="region", hue="iscustomer", data=data, palette=["#C3D350", "#5e2f78"])
```
...
customers_age = data[data["iscustomer"] == 1]["age"]
non_customers_age = data[data["iscustomer"] == 0]["age"]

# Calculate mean ages
mean_cust_age = customers_age.mean()
mean_non_cust_age = non_customers_age.mean()

# Create side-by-side subplots with shared X and Y axes
fig, axes = plt.subplots(1, 2, figsize=(12, 6), sharex=True, sharey=True)

# Plot 1: Customers Age Distribution (Fixed: ax=axes[0])
sns.histplot(
    data=data[data["iscustomer"] == 1],
    x="age",
    bins=20,
    kde=True,
    color="#C3D350",
    edgecolor="black",
    alpha=0.6,
    ax=axes[0],  # <-- Check this line in your code
)
axes[0].axvline(
    mean_cust_age,
    color="black",
    linestyle="--",
    linewidth=2,
    label=f"Mean Age: {mean_cust_age:.1f}",
)
...
# Plot 2: Non-Customers Age Distribution (Fixed: ax=axes[1])
sns.histplot(
    data=data[data["iscustomer"] == 0],
    x="age",
    bins=20,
    kde=True,
    color="#5e2f78",
    edgecolor="black",
    alpha=0.6,
    ax=axes[1],  # <-- Check this line in your code
)
axes[1].axvline(
    mean_non_cust_age,
    color="black",
    linestyle="--",
    linewidth=2,
    label=f"Mean Age: {mean_non_cust_age:.1f}",
)
axes[1].set_title("Non-Customer Age Distribution")
